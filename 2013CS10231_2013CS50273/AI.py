...
DealerBJA = p
DealerBJF = (1-p)/9
# DealerProb[8][21] -= DealerBJF
# DealerProb[9][21] -= DealerBJA 
# print DealerProb
...
for i in xrange(21):
	PosAce.append([i,11+i])

def PossibleScores(inputState):
	posscores=[inputState.FixedCardVal]
	if inputState.NumAce==0:
		posscores = [inputState.FixedCardVal]
	else:
		posscores = [inputState.FixedCardVal + inputState.NumAce,inputState.FixedCardVal+ 10+inputState.NumAce]
...
def CalculateWinningStatic(DealerI,elem,numcards):
	arr= DealerProb[DealerI]
	ans=-1
	if (0<elem<21):
		ans=arr[22]
...
def DecideMove(s):
	...
	if s in AllStatePolicies:
		...
		AllStatePolicies[s]=[("S",-1.0) for i in xrange(10)]
		return AllStatePolicies[s]	
	else:
		possibilities= ["H","S"]
		# Considering S
		...
		for elem in xrange(10):
			WinningAmountS.append(CalculateWinningStatic(elem, presscore,s.NumCards))
...
def CalcWinningAmtDoubled(InputState):
	...
	for i in xrange(10):
		ans[i]=2*InputState[1]*CalculateWinningStatic(i,sc,InputState[0].NumCards)
	return ans
...
def CalculateWinningSplitA(state):
	...
	for i in xrange(10):
		ans[i]=2*state[1]*CalculateWinningStatic(i, sc,3)
	return ans
	...
	if cards1!=cards2:
		...
		if cards1==11:
			...
			if den>0:
				...
				# print "IN DENOM NEG CASE"
				expscore = map (lambda x: 99999 if (x>0) else -99999,TotalSofar)
				# TODO change
				# expscore = [9999]*10	
...
for i in xrange(5, 20):
	...
	f1.write(str(i)+"\t")
	for b in xrange(0,len(ArrAns[-1])-1):
		f1.write(ArrAns[-1][b][0]+" ")
	f1.write(ArrAns[-1][-1][0])
	f1.write("\n")	
